...
CARD_INFO_FILE="$BOT_DIR/card_info.txt"
...
if [[ -f "$CARD_INFO_FILE" ]]; then
  CARD_BACKUP="$(mktemp)"
  cp "$CARD_INFO_FILE" "$CARD_BACKUP"
elif [[ -f "$LEGACY_CARD_INFO_FILE" ]]; then
  CARD_BACKUP="$(mktemp)"
  cp "$LEGACY_CARD_INFO_FILE" "$CARD_BACKUP"
else
  CARD_BACKUP=""
fi
...
cleanup() {
  ...
  if [[ -n "${CARD_BACKUP:-}" && -f "${CARD_BACKUP:-}" ]]; then
    rm -f "$CARD_BACKUP"
  fi
}
trap cleanup EXIT
...
if [[ -n "$CARD_BACKUP" && -f "$CARD_BACKUP" ]]; then
  cp "$CARD_BACKUP" "$CARD_INFO_FILE"
  chmod 600 "$CARD_INFO_FILE"
fi
