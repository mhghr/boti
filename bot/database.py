...
def init_db():
    ...
    with engine.begin() as conn:
        ...
        conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS org_last_billed_usage_bytes BIGINT DEFAULT 0"))
        conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS org_wallet_negative_limit INTEGER DEFAULT 0"))
        conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS org_negative_alert_sent BOOLEAN DEFAULT FALSE"))
